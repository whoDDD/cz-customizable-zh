...
module.exports = {
  getQuestions(config, cz) {
    ...
    messages.subject =
      messages.subject || 'Write a SHORT, IMPERATIVE tense description of the change:\n'
    messages.body =
      messages.body ||
      'Provide a LONGER description of the change (optional). Use "|" to break new line:\n'
    messages.breaking = messages.breaking || 'List any BREAKING CHANGES (optional):\n'
    messages.footer =
      messages.footer || 'List any ISSUES CLOSED by this change (optional). E.g.: #31, #34:\n'
    messages.confirmCommit =
      messages.confirmCommit || 'Are you sure you want to proceed with the commit above?'
    ...
    let questions = [
      ...
      },
      {
        ...
        filter(value) {
          ...
          return (
            (upperCaseSubject ? value.charAt(0).toUpperCase() : value.charAt(0).toLowerCase()) +
            value.slice(1)
          )
        }
      },
